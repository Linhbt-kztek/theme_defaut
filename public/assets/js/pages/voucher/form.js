...
function submit() {
    ...
    if (!has_error) {
        voucher_name = $('input[name=name]').val();
        if (voucher_name == '' || voucher_name == undefined) {
            has_error = true;
    ...
    if (!has_error) {
        voucher_count = $('input[name=count]').val();
        if (voucher_count == '' || voucher_count == undefined) {
            has_error = true;
            message = 'Số lượng voucher không được để trống!';
        } else {
            if (voucher_count == 0) {
                has_error = true;
