    ...
    a.fn.spartanMultiImagePicker = function(i) {
        ...
        function s(i, t, d) {
            ...
            if (t.files && t.files[0]) {
                ...
                if (!new RegExp(`(.*?).(${l})$`).test(c) && '' !== l) {
                    if (a(d).find('img[data-spartanindeximage="' + r + '"]').is(':visible')) {
                        a(d).find('img[data-spartanindexi="' + r + '"]').hide();
                    }
                ...
                fileReader.onload = function(e) {
                    a(d).find('img[data-spartanindexi="' + r + '"]').hide();
                    a(d).find('a[data-spartanindexremove="' + r + '"]').show();
                    a(d).find('img[data-spartanindeximage="' + r + '"]').attr('src', e.target.result);
                    a(d).find('img[data-spartanindeximage="' + r + '"]').show();
                    i.onRenderedPreview.call(this, r);

                    if (i.directUpload.status === 1) {
                        var n = a(t).data('spartanindexinput');
                        var formData = new FormData();
                        var file = t.files[0];
                        var additionalParams = i.directUpload.additionalParam;

                        a(d).find('[data-spartanindexloader="' + n + '"]').css('display', 'flex');
                        formData.append('file', file);

                        if (additionalParams !== undefined) {
                            a.each(additionalParams, function(a, i) {
                                formData.append(a, i);
                            });
                        }
                        ...
                        a.ajax({
                            ...
                            success: function(e, d, r) {
                                a(d).find('[data-spartanindexloader="' + n + '"]').css('display', 'none');
                                if (i.directUpload.success !== undefined) {
                                    i.directUpload.success(this, e, d, r);
                                }
                            },
                            error: function(e, d, r) {
                                a(d).find('[data-spartanindexloader="' + n + '"]').css('display', 'none');
                                if (i.directUpload.error !== undefined) {
                ...
                var isVisible = a(d).find('img[data-spartanindeximage="' + r + '"]').is(':visible');
                var isFirstVisibleImage = !isVisible && n === 0;
                if (isFirstVisibleImage) {
                    n++;
                    if (i.maxCount === '') {
                        o(i, d);
                    } else if (i.maxCount !== '' && n < i.maxCount) {
                        o(i, d);
                    }
                }
            }
